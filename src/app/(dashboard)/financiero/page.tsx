...
export default async function FinancieroPage() {
  ...
  const [rows, players] = await Promise.all([
    loadSalaryRows(session.tenantId),
  ...
  return (
    <div className="grid gap-8">
      <PageHeader
        ...
        title="Financiero"
        subtitle="Costo salarial por jugador y por minuto jugado."
      />

      {rows.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-3">
          <SummaryCard label="Jugadores con salario cargado" value={String(rows.length)} />
          <SummaryCard label="Costo total de temporada (todas las monedas mezcladas)" value={Math.round(totalSeasonCost).toLocaleString("es-PY")} />
          <SummaryCard
            label="Costo/minuto promedio (con minutos cargados)"
